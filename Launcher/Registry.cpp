 ...
 * @return bool  : 
 */
bool CRegistry::Get(const char* key,const char* name, CString& data)
{
	CRegKey	regKey;
	TCHAR value[255];
	ULONG len=255;

	if (regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+key,KEY_EXECUTE)!=ERROR_SUCCESS)
		return false;
 ...
 * @return bool  : 
 */
bool CRegistry::Set(const char* key,const char* name,const CString& data)
{
	CRegKey	regKey;
	if (regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+key)!=ERROR_SUCCESS)
	{
		if (Create(HKEY_LOCAL_MACHINE,m_startKey+key)!=ERROR_SUCCESS || 
			regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+key)!=ERROR_SUCCESS)
			return false;
	}
 ...
 * @return bool  : 
 */
bool CRegistry::Get(const char* key,const char* name,int& data)
{
	CRegKey	regKey;
	DWORD   dwValue=0;
 ...
 * @return bool  : 
 */
bool CRegistry::Set(const char* key,const char* name,  int  data)
{
	CRegKey	regKey;
	if (regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+key,KEY_READ)!=ERROR_SUCCESS)
	{
		if (Create(HKEY_LOCAL_MACHINE,m_startKey+key)!=ERROR_SUCCESS || 
			regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+key)!=ERROR_SUCCESS)
			return false;
 ...
 * @return bool  : 
 */
bool CRegistry::Get(const char* key,const char* name, double& data)
{
	CString strg;
 ...
 * @return bool  : 
 */
bool CRegistry::Set(const char* key,const char* name, double  data)
{
	CString strg;
...
* @return double  : 
*/
void CRegistry::Delete(const char *key, const char *name)
{
	CRegKey	regKey;

	if (regKey.Open(HKEY_LOCAL_MACHINE,m_startKey+CString(key))==ERROR_SUCCESS)
...
* @return double  : 
*/
void CRegistry::Delete(const char *key)
{
	CRegKey	regKey;
	if (regKey.Open(HKEY_LOCAL_MACHINE,m_startKey)==ERROR_SUCCESS)
